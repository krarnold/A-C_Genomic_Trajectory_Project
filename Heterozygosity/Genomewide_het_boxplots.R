...
data_long <- data %>%
  ...
  mutate(
    population = paste0(popdir, replicate)
  ) %>%
  transmute(
    population,
    treatment = popdir,
...
# For each trajectory, compare each post-founding generation to generation 0.
# Holm-Bonferroni correction applied separately within each trajectory:
# (1) each trajectory constitutes its own family of 3 ordered comparisons
#     against a common baseline, so corrections are not pooled across trajectories;
...
# Build comparisons: for each treatment, compare all non-zero generations to 0
...
# Apply Holm correction separately within each trajectory
labels_df <- labels_df %>%
  group_by(treatment) %>%
  mutate(
...
# Place significance labels above the top box in each facet
y_pos <- het_summary %>%
  group_by(treatment) %>%
...
# ---- PLOT 2: Publication SVG (all timepoints + significance labels) ----
...
ggsave(
  "Mean_Heterozygosity_Boxplot.tiff",
