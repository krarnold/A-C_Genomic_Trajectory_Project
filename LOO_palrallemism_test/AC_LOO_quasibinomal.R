# ===============================================================
#  A>C Leave-One-Out Parallelism Analysis
#  (FDR < 0.05 and |Δp| >= 0.02, random controls)
#
#  Inputs:
#     Long_format_SNP_Data.csv (SNP counts)
#  Outputs:
#     AC_LOO_medians.csv  (median Δp for targets vs random controls)
# ===============================================================
...
data_long <- read.csv("Long_format_SNP_Data.csv", header = TRUE) %>%
  filter(treatment == "A>C", chr != "4") %>%  # exclude chr 4
  mutate(
...
for (rep in replicates) {
  ...
  rm(train_data, test_data, results_train, merged,
     delta_train, targets, nonsig, matched_controls, sig_table, delta_test)
  gc()
}
...
write.csv(median_table, "AC_LOO_medians.csv", row.names = FALSE)
message("Finished A>C LOO analysis (FDR<0.05, |Δp|>=0.02, chr 4 excluded, random controls). Wrote AC_LOO_medians.csv")
